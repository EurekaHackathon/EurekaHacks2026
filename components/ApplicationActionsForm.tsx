...
const ACTION_CONFIG: Record<string, {
    ...
    rejected: {
        ...
        confirmDescription: "A rejection email will be sent. You can undo this by resetting the status.",
    ...
    waitlisted: {
        ...
        confirmDescription: "A waitlist email will be sent. You can undo this by resetting the status.",
    ...
    accepted: {
        ...
        confirmDescription: "An acceptance email will be sent. You can undo this by resetting the status.",
    ...
    submitted: {
        ...
        confirmDescription: "Status goes back to 'Submitted'. No email will be sent.",
...
export function ApplicationActionsForm({ id }: { id: number }) {
    ...
    const updateStatus = async (status: string) => {
        ...
        if (response.ok) {
            router.refresh();
            toast({ variant: "success", title: "Success", description: "Application status updated" });
        } else {
            toast({ variant: "error", title: "Error", description: "Failed to update application status" });
        }
    };
        ...
        <div className="mt-8">
            ...
            <Dialog open={pendingAction !== null} onOpenChange={(open) => { if (!open) setPendingAction(null); }}>
                <DialogContent className="bg-[#151c2b] border-gray-700 text-secondary-50 max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-secondary-50">{config?.confirmTitle}</DialogTitle>
                    </DialogHeader>
