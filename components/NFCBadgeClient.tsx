...
import { MEAL_EVENTS, type MealEventKey } from "@/lib/events";
...
type LookupResult = {
  ...
  firstName: string;
  lastName: string;
} & Record<MealEventKey, boolean>;

// ── Helpers ───────────────────────────────────────────────────────────────────

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const INSTRUCTIONS_URL = "https://eurekahacks.ca/nfc";

async function writeNFC(url: string): Promise<void> {
  if (!("NDEFReader" in window)) throw new Error("Use Android Chrome for NFC.");
  const ndef = new NDEFReader();
  await ndef.write({ records: [{ recordType: "url", data: url }] });
}

function describeNfcError(err: unknown): string {
  if (err instanceof DOMException) {
    switch (err.name) {
      ...
      case "NetworkError":
        return "Couldn't write to tag. Hold it still against the phone and retry.";
      case "AbortError":
        return "Write cancelled.";
      case "InvalidStateError":
        return "NFC is busy — stop the scanner before writing.";
    }
    return err.message || `Write failed (${err.name})`;
  }
  if (err instanceof Error) return err.message || "Couldn't write to tag";
  return "Couldn't write to tag";
}

function extractChipId(raw: string): string | null {
  try {
    const u = new URL(raw);
    const chip = u.searchParams.get("chip");
    if (chip && UUID_RE.test(chip)) return chip;
  } catch {
...
function useNFCScanner(onRead: (val: string) => void) {
  ...
  const start = useCallback(async () => {
    ...
    try {
      const ndef = new NDEFReader();
      ndef.onreading = (e) => {
        console.log("[NFC] reading", e);
        let decoded = "";
        for (const r of e.message.records) {
          if (!r.data) continue;
          if (r.recordType === "text") {
            decoded = new TextDecoder(r.encoding ?? "utf-8").decode(r.data);
            break;
          }
          if (r.recordType === "url" || r.recordType === "absolute-url") {
            decoded = new TextDecoder().decode(r.data);
            break;
          }
        }
        onRead(decoded);
      ...
      ndef.onreadingerror = () => {
        toast({
          ...
          description: "Try again, or check the tag isn't damaged.",
        });
      };
      await ndef.scan({ signal: ac.signal });
      toast({ variant: "default", title: "Scanning..." });
      setScanning(true);
...
const MEALS: ReadonlyArray<{ key: MealEventKey; label: string }> = MEAL_EVENTS;
...
function ReadTab() {
  ...
  const onChip = useCallback(async (raw: string) => {
    const chipId = extractChipId(raw);
    if (!chipId) {
      toast({ variant: "error", title: "Tag not found" });
      return;
    }
    setLoading(true);
    setResult(null);
    try {
      const res = await fetch(
        `/api/nfc/lookup?chip=${encodeURIComponent(chipId)}`,
      );
      if (!res.ok) {
        toast({ variant: "error", title: "Tag not found" });
        return;
  ...
  const toggleMeal = useCallback(
    async (key: keyof LookupResult, next: boolean) => {
      if (!result) return;
      const userId = result.userId;
      setResult({ ...result, [key]: next });
      setPendingMeals((s) => new Set(s).add(String(key)));
      try {
        const res = await fetch("/api/nfc/event", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId, name: key, checked: next }),
        });
        if (!res.ok) {
          setResult((r) => (r ? { ...r, [key]: !next } : r));
          toast({
            variant: "error",
            title: "Update failed",
            description: (await res.text()) || "Server error",
          });
        }
      } catch {
        setResult((r) => (r ? { ...r, [key]: !next } : r));
    ...
    <div className="flex flex-col gap-4 w-full">
      {supported === false && (
        <div className="p-3 bg-red-950 border border-red-700 rounded-lg text-red-400 text-xs flex gap-2">
          <Icon icon="fluent:warning-24-filled" className="shrink-0 mt-0.5" />{" "}
          Web NFC not supported. Use Android Chrome.
        </div>
      )}

      <div className="flex flex-col items-center gap-3 py-2">
        <div
          className={`flex items-center justify-center w-32 h-32 rounded-full border-4 transition-colors ${scanning ? "border-blue-400 bg-blue-950" : "border-gray-700 bg-[#0d1120]"}`}
        >
          <Icon
            icon="fluent:nfc-28-filled"
            className={`text-6xl transition-colors ${scanning ? "text-blue-400 animate-pulse" : "text-gray-600"}`}
          />
        </div>
        ...
        <div className="bg-[#0d1120] border border-gray-700 rounded-xl overflow-hidden">
          ...
          <div className="flex flex-col divide-y divide-gray-800">
            {MEALS.map((m) => {
              ...
              const pending = pendingMeals.has(String(m.key));
              return (
                <label
                  key={String(m.key)}
                  className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-[#141a2b] transition-colors"
                >
                  <input
                    type="checkbox"
                    checked={checked}
                    disabled={pending}
                    onChange={(e) => toggleMeal(m.key, e.target.checked)}
                    className="w-4 h-4 accent-secondary-500 cursor-pointer disabled:opacity-50"
                  />
                  <span className="text-gray-300 text-sm flex-1">{m.label}</span>
                  {pending && (
                    <Icon
                      icon="fluent:spinner-ios-20-filled"
                      className="text-gray-500 animate-spin text-sm"
                    />
                  )}
                </label>
              );
...
function CreateTab() {
  ...
  useEffect(() => {
    ...
    debounce.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/nfc/search?q=${encodeURIComponent(query)}`,
        );
  ...
  const handleCreate = async () => {
    ...
    const url = `${INSTRUCTIONS_URL}?chip=${chipId}`;
    try {
      try {
        await writeNFC(url);
      } catch (err) {
        toast({
          ...
          description: describeNfcError(err),
        });
        return;
      }
      const res = await fetch("/api/nfc/assign", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chipId, userId: selected.userId }),
      });
      if (!res.ok) {
        toast({
          variant: "error",
          title: "Tag written, but registration failed",
          description: `${(await res.text()) || "Server error"} — retry to register.`,
        });
        return;
      }
      setSaved(true);
      toast({
        variant: "success",
        title: "Done",
        description: `Chip ${chipId.slice(0, 8)}… → ${selected.firstName} ${selected.lastName}`,
      });
    } finally {
      setSaving(false);
    }
      ...
      {selected ? (
        <>
          <div className="flex items-center gap-3 bg-[#0d1120] border border-secondary-700 rounded-xl px-4 py-3">
            ...
            <span
              className={`text-xs px-2 py-0.5 rounded-full shrink-0 ${STATUS_COLORS[selected.status] ?? "bg-gray-100 text-gray-600"}`}
            >
              {selected.status}
            </span>
