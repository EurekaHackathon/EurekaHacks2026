...
const scheduleItems: ScheduleItem[] = parseScheduleItems(
  scheduleData as ScheduleJsonItem[]
);
...
export default function SchedulePage() {
  ...
  const halfHourLines = Array.from(
    { length: TOTAL_MINUTES / 30 + 1 },
    (_, i) => addMinutes(SCHEDULE_START, i * 30)
  );
  ...
  return (
    <div className="p-6 pt-16 lg:pt-6">
      ...
      <p className="mb-6 text-secondary-300">
        Friday 5:00 PM – Saturday 10:00 PM
      </p>

      <div className="overflow-x-auto">
        <div className="min-w-[1100px]">
          <div
            className="overflow-auto rounded-xl border border-secondary-800 bg-secondary-950/70"
            style={{ maxHeight: "calc(100vh - 14rem)" }}
          >
            {/* ----- column headings ----- */}
            <div
              className="sticky top-0 z-20 mb-2 grid border-b border-secondary-800 bg-secondary-950/95"
            ...
            {/* ----- main grid ----- */}
            <div className="grid px-2" style={{ gridTemplateColumns }}>
              ...
              >
                {hourLines.map((time, index) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`-translate-y-1/2 whitespace-nowrap pr-3 text-right text-xs text-secondary-400 ${
                          isTerminal ? "hidden" : ""
                        }`}
                      >
                        {formatTimelineLabel(time, false)}
                      </div>
                    </div>
                  );
                })}
              ...
              {columns.map((column, columnIndex) => {
                ...
                return (
                  <div
                    key={column.id}
                    className={`relative border-secondary-800 ${
